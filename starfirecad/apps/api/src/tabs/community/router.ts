...
communityRouter.post('/', async (req, res) => {
  ...
  await prisma.membership.create({ data: { communityId: community.id, userId: user.id, role: 'COMMUNITY_OWNER' as any } });
...
communityRouter.get('/', async (req, res) => {
  ...
  res.json({ communities: memberships.map((m: any) => ({ ...m.community, role: m.role })) });
});
...
communityRouter.post('/:id/members', async (req, res) => {
  ...
  const member = await prisma.membership.upsert({
    ...
    create: { userId: parse.data.userId, communityId: id, role: parse.data.role as any },
    update: { role: parse.data.role as any }
  });
