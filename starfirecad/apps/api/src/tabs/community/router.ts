...
communityRouter.post('/', async (req, res) => {
  ...
  if (existing) return res.status(409).json({ error: 'Community name or slug in use' });
  const community = await prisma.community.create({ data: { name, slug, createdById: user.id, settings: {} } });
  await prisma.membership.create({ data: { communityId: community.id, userId: user.id, role: CommunityRole.COMMUNITY_OWNER } });
...
communityRouter.patch('/:id', async (req, res) => {
  ...
  if (!membership || (membership.role !== CommunityRole.COMMUNITY_OWNER && membership.role !== CommunityRole.COMMUNITY_ADMIN && membership.role !== CommunityRole.COMMUNITY_MANAGER)) {
  ...
  if (parse.data.settings) data.settings = parse.data.settings;
...
communityRouter.post('/:id/members', async (req, res) => {
  ...
  const user = req.user;
  const { id } = req.params;
  const roleSchema = z.object({ userId: z.string().uuid(), role: z.nativeEnum(CommunityRole) });
  ...
  if (!adminMembership || (adminMembership.role !== CommunityRole.COMMUNITY_OWNER && adminMembership.role !== CommunityRole.COMMUNITY_ADMIN)) {
    return res.status(403).json({ error: 'Forbidden' });
  }
